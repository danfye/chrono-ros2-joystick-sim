...
void ExpectCommandNear(const DriverCommand& command, double steering, double throttle, double braking) {
    EXPECT_NEAR(command.steering, steering, 1e-9);
    EXPECT_NEAR(command.throttle, throttle, 1e-9);
    EXPECT_NEAR(command.braking, braking, 1e-9);
}
