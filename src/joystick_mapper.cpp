...
JoystickMapper::JoystickMapper(const JoystickMapperConfig& config) : config_(config) {
...
double JoystickMapper::ReadAxis(
    ...
    const double raw = ApplyDeadzone(Clamp(static_cast<double>(axes[static_cast<std::size_t>(index)]), -1.0, 1.0), config_.deadzone);
...
DriverCommand JoystickMapper::MapAxes(const std::vector<float>& axes, bool* ok) const {
    ...
    mapped.steering = ReadAxis(axes, config_.steering_axis, config_.invert_steering, config_.steering_scale, ok);

    const bool independent_pedals = config_.throttle_axis >= 0 || config_.brake_axis >= 0;
    if (independent_pedals) {
        mapped.throttle =
            std::max(0.0, ReadOptionalAxis(axes, config_.throttle_axis, config_.invert_throttle, config_.throttle_scale, ok));
        mapped.braking =
            std::max(0.0, ReadOptionalAxis(axes, config_.brake_axis, config_.invert_brake, config_.brake_scale, ok));
    } else {
        const bool invert_combined_axis = config_.invert_throttle || config_.invert_brake;
        const double throttle_brake = ReadAxis(axes, config_.throttle_brake_axis, invert_combined_axis, 1.0, ok);
