...
public:
    ...
    DriverCommand UpdateFromJoy(const std::vector<float>& axes, const std::vector<int>& buttons, double now_seconds);
...
private:
    ...
    double ReadAxis(const std::vector<float>& axes, int index, bool invert, double scale, bool* ok) const;
    double ReadOptionalAxis(const std::vector<float>& axes, int index, bool invert, double scale, bool* ok) const;
